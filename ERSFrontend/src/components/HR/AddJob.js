...
const AddJob = (props) => {
	...
	const updateProfile = (event) => {
		...
		axios
			.post(`${process.env.REACT_APP_API}/addJobOpening`, { ...jobData, companyID })
			.then((response) => {
				...
				if (response.status === 200) {
					...
					alert("Could not Add Job Opening! Please try again");
	...
	return (
		<body style={{ backgroundColor: "#1f1e2e", color: "black" }}>
			{/* {page === "AddJob" ? <Nav/> : null} */}
			<Nav />
			<div
				style={{
					marginLeft: page === "AddJob" ? "10vw" : "14vw",
					marginTop: "2em",
					marginBottom: "3em",
				}}
			>
